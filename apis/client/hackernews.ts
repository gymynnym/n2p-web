...
const generateHackerNewsPodcast = async (
  ...
  const response = await clientAPI.post('hackernews/podcasts/generate', { json, signal, timeout: 100_000 });
  const { body } = response;
  if (!body) {
...
export { generateHackerNewsPodcast };
